...
void SPDKIO::BlockController::SpdkBdevEventCallback(enum spdk_bdev_event_type type, struct spdk_bdev *bdev, void *event_ctx) {
    LOG(Helper::LogLevel::LL_Error, "SpdkBdevEventCallback: supported bdev event type %d\n", type);
}
...
void SPDKIO::BlockController::SpdkBdevIoCallback(struct spdk_bdev_io *bdev_io, bool success, void *cb_arg) {
    ...
    if (success) {
        ...
        LOG(Helper::LogLevel::LL_Error, "SpdkBdevIoCallback: I/O failed %p\n", currSubIo);
        spdk_app_stop(-1);
    }
}
...
void SPDKIO::BlockController::SpdkIoLoop(void *arg) {
    ...
    while (!ctrl->m_ssdSpdkThreadExiting) {
        if (ctrl->m_submittedSubIoRequests.try_pop(currSubIo)) {
            ...
            if (rc && rc != -ENOMEM) {
                LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::SpdkStart %s failed: %d, shutting down, offset: %ld\n",
...
void SPDKIO::BlockController::SpdkStart(void *arg) {
    ...
    if (rc) {
        LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::SpdkStart: spdk_bdev_open_ext failed, %d\n", rc);
    ...
    if (ctrl->m_ssdSpdkBdevIoChannel == NULL) {
        LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::SpdkStart: spdk_bdev_get_io_channel failed\n");
        spdk_bdev_close(ctrl->m_ssdSpdkBdevDesc);
        ctrl->m_ssdSpdkThreadStartFailed = true;
        spdk_app_stop(-1);
        return;
...
bool SPDKIO::BlockController::Initialize(std::string filePath, int batchSize) {
    ...
    m_numInitCalled++;
    LOG(Helper::LogLevel::LL_Info, "SPDKIO::BlockController::Initialize(%s, %d)\n", filePath.c_str(), batchSize);
    ...
    if (m_useMemImpl) {
        ...
        if (m_numInitCalled == 1) {
            ...
            if (m_ssdSpdkThreadStartFailed) {
                LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::Initialize failed\n");
                return false;
            }
	    m_filePath = filePath;
...
bool SPDKIO::BlockController::GetBlocks(AddressType* p_data, int p_size) {
    ...
    if (m_useMemImpl || m_useSsdImpl) {
        ...
        LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::GetBlocks failed\n");
        return false;
    }
}
...
bool SPDKIO::BlockController::ReleaseBlocks(AddressType* p_data, int p_size) {
    if (m_useMemImpl || m_useSsdImpl) {
        ...
        LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::ReleaseBlocks failed\n");
        return false;
    }
}
...
bool SPDKIO::BlockController::ReadBlocks(AddressType* p_data, std::string* p_value, const std::chrono::microseconds &timeout) {
    if (m_useMemImpl) {
        ...
        LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::ReadBlocks single failed\n");
        return false;
    }
}
...
bool SPDKIO::BlockController::ReadBlocks(std::vector<AddressType*>& p_data, std::vector<std::string>* p_values, const std::chrono::microseconds &timeout) {
    if (m_useMemImpl) {
        ...
        LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::ReadBlocks batch failed\n");
        return false;
    }
}
...
bool SPDKIO::BlockController::ShutDown() {
    ...
    if (m_useMemImpl) {
        ...
        LOG(Helper::LogLevel::LL_Error, "SPDKIO::BlockController::ShutDown failed\n");
        return false;
    }
}
